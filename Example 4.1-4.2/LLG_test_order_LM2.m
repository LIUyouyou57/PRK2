    % 基于 arXiv:2206.02882 (保模1和保能量耗散都加了拉格朗日乘子)
    % 区域: [-1/2, 1/2]^2
    % BC: Zero Neumann
...
% --- [新增] 定义梯形积分权重 W ---
    ...
    % 初始能量
    Energy = zeros(Nt+1, 1);
    Energy(1) = calc_energy(m1, m2, m3, hx, hy, L, W);
...
% --- [修改] 加上权重 W ---
        ...
        func_xi = @(xi) energy_wrapper(xi, m1_hat, m2_hat, m3_hat, hx, hy, L, W) - E_target;
        ...
        Energy(n+1) = calc_energy(m1, m2, m3, hx, hy, L, W);
    ...
    load("LLG_BDF4_N_64_T_0p01024.mat");
    error_L2 =  (m1_ex-m1).^2 + (m2_ex-m2).^2 + (m3_ex-m3).^2;
    Error_L2(i) =  sqrt( h^2*sum(error_L2(:)) );
    ...
    data(1).error = Error_L2;
    data(1).order = order_L2_1;
...
function E = calc_energy(m1, m2, m3, hx, hy, L, W) 
    Lm1 = reshape(L * m1(:), size(m1));
    Lm2 = reshape(L * m2(:), size(m2));
    Lm3 = reshape(L * m3(:), size(m3));
...
function E = energy_wrapper(xi, m1h, m2h, m3h, hx, hy, L, W)
    t1 = m1h + xi; t2 = m2h + xi; t3 = m3h + xi;
    nt = sqrt(t1.^2 + t2.^2 + t3.^2);
    E = calc_energy(t1./nt, t2./nt, t3./nt, hx, hy, L, W);
end
