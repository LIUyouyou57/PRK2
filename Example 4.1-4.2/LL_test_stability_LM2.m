    ...
    T  = 0.002; 
    Nt = round(T/dt);
    load("LL_BDF4_N_48_T_0p002.mat");
    
    gamma = 1.0;
    beta  = -1.0; % 设为 0 则匹配 LLG
    S     = 0; 
    %% ================= Grid =================
...
% --- [新增] 定义梯形积分权重 W ---
    ...
    data(i).m = [m1_n(:); m2_n(:); m3_n(:)];
    end
    
    order = log(Error_L2(1:6)./Error_L2(2:7))/log(2)
    data(1).error = Error_L2;
    data(1).order = order;

    figure('Color', 'w', 'Name', 'Work-Precision Diagram');
    loglog(wall_times, Error_L2, '-o', 'LineWidth', 1.5, 'DisplayName', 'LM2');
